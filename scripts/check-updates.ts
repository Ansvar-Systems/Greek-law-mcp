#!/usr/bin/env tsx
/**
 * Greek Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.et.gr';
const PORTAL_NAME = 'Ethniko Typografeio (National Printing House)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/greek-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Greek Law MCP — Data Freshness Check');
