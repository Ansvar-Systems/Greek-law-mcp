/**
 * get_greek_implementations — Find Greek statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetGreekImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface GreekImplementationResult {
  document_id: string;
...
export async function getGreekImplementations(
  db: InstanceType<typeof Database>,
  input: GetGreekImplementationsInput,
): Promise<ToolResponse<GreekImplementationResult[]>> {
  try {
