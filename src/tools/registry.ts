/**
 * Tool registry for Greek Law MCP Server.
...
import { getGreekImplementations, type GetGreekImplementationsInput } from './get-greek-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Greek Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Greek legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Greek statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Greek statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Greek legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Greek statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Greece?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Greek legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Greek statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Greek statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_greek_implementations',
    description:
      'Find all Greek statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Greek statutes. ' +
      'Note: Greece does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Greek legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_greek_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Greek legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Greek statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Greek statute or provision. ' +
      ...
      'Note: Greece is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Greek statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_greek_implementations':
          result = await getGreekImplementations(db, args as unknown as GetGreekImplementationsInput);
          break;
