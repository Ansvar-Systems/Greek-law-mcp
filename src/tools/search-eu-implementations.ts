/**
 * search_eu_implementations — Search EU directives/regulations referenced by Greek legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_greek_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as greek_statute_count
  ...
  if (input.has_greek_implementation) {
    sql += ' HAVING greek_statute_count > 0';
  }
